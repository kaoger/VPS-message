...
測試涵蓋正式環境必要設定、測試路由關閉、Webhook 簽章、表單驗證、Token 過期／竄改、並行重複提交、API 失敗、首次一般訊息邀請、四個服務入口與邀請去重、無資料庫模式及頻率限制。
...
修改連結綁定客戶、案件與資料版本，有效兩小時；含個資存取權限，請勿轉傳。過期或舊分頁請在 Messenger 傳「修改需求」取得最近一筆案件的新連結。若是另一個案件，傳「新增需求」取得空白表單；更早的案件可由該筆摘要下的有效連結修改。

資料庫模式會讀取最新資料，更新時同時檢查案件 ID、Messenger 使用者與原始 answers，避免兩個修改分頁互相覆寫。版本記號 `_revision` 存於既有 answers JSON，沒有新增案件欄位；正式驗收需核對 answers 為支援 JSON 相等比較的 jsonb 欄位。重啟後，修改連結仍可使用（密鑰未更換且未過期）。

webform 的自動表單邀請每個 Messenger ID 永遠只發一次：四個服務破冰入口維持服務預填；若客戶沒有點破冰而先傳一般文字，則發通用空白表單。Meta 既有 FAQ 問句不觸發通用邀請。「新增需求」可手動再索取空白表單。要讓一次性紀錄在重啟後保留，部署前需在 Supabase SQL Editor 執行一次 [`ops/automatic-invite-registry.sql`](ops/automatic-invite-registry.sql)，並確認 Supabase Data API 有開放 `public` schema；若沒有設定 Supabase，僅能在目前程序執行期間去重。
...
- 表單送出鎖與流量限制存於記憶體，重啟會清空。自動邀請使用 `bot_invite_registry` 主鍵去重，跨重啟仍有效；手動「新增需求」不受限制。
